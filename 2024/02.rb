...
class RedNosedReports
  ...
  def safeish_reports_count_old
    max_change = 3
    min_change = -3
    acceptable = [-3, -2, -1, 1, 2, 3] # exclude zero

    parse_input.reduce(0) do |sum, row|
      lag = nil
      dropped = 0

      changes = row.each_cons(2).map do |a, b|
        change = (a - b)
        change_lag = lag.nil? ? nil : (lag - b)

        result = if acceptable.include?(change)
                   change
                 else
                   if dropped < 1 && acceptable.include?(change_lag)
                     dropped += 1
                     change_lag
                   else
                     change
      ...
      if safe_criteria.include?(changes.uniq.sort)
        puts "safe"
        sum + 1
      else
        puts "unsafe"
  ...
  def safeish_reports_count
    parse_input.reduce(0) do |sum, row|
      if safeish_report?(row, acceptable_changes: [1, 2, 3])
        sum + 1
      elsif safeish_report?(row, acceptable_changes: [-3, -2, -1])
        sum + 1
      else
        puts "unsafe!"
        sum
  ...
  def safeish_report?(row, acceptable_changes: [1, 2, 3])
    safe_criteria = [acceptable_changes]
      .map(&:sort)
      .map { |x| [x.combination(1), x.combination(2), x.combination(3)].map(&:to_a) }
      .flatten(2)
    ...
    changes = row.each_cons(2).map do |a, b|
      ...
      result = if acceptable_changes.include?(change)
                 change
               else
                 if dropped < 1 && acceptable_changes.include?(change_lag)
                   dropped += 1
                   change_lag
                 else
                   change
    ...
    if safe_criteria.include?(changes.uniq.sort)
      # puts "#{changes} safe" # #{safe_criteria.inspect}"
      true
    else
      # puts "#{changes} unsafe" # #{safe_criteria.inspect}"
      false
    end
  end
  ...
  def safe_reports_count
    ...
    parse_input.reduce(0) do |sum, row|
      ...
      if safe_criteria.include?(changes)
        sum + 1
      else
        sum
      end
...
RSpec.describe "Day 1" do
  subject { RedNosedReports.new(input) }
  ...
  context "with sample input" do
    ...
    it "determines safe reports count" do
      expect(subject.safe_reports_count).to eq(2)
    end

    it "determines safeish reports count" do
      expect(subject.safeish_reports_count).to eq(4)
    end
  end

  context "with actual input" do
    it "determines safe reports count" do
      expect(subject.safe_reports_count).to eq(442)
    end
